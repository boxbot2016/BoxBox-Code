...
void loop() {
  while (retrieve == 0) {
    ...
    if ((val5 > threshold && val5 > maxLeft && val5 > maxRight) || (val6 > threshold && val6 > maxLeft && val6 > maxRight) || (val7 > threshold && val7 > maxLeft && val7 > maxRight)) { // Go forward when middle sensor detects most IR
  ...
  if (retrieve == 1 && stopTurning == 1) { //if on retrieval phase, turn around
    ...
    if (netLeftLoop > 0) {
      ...
      while (forwardLoopCount > 0) { // Go forward toward starting location
        ...
        digitalWrite(rightForward, LOW);
        analogWrite(rightBackward, velocity);
        delay(50); // Have the robot move forward for a decent amount of time instead of being so jittery side to side
        forwardLoopCount = forwardLoopCount - 1;
        Serial.print("forwardLoopCount: ");
        Serial.println(forwardLoopCount);
      }
      stopTurning = 0;
      digitalWrite(leftForward, LOW);
