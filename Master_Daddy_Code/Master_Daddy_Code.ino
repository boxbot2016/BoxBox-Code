...
void loop() {
  ...
  if (retrieve == 0) {
    ...
    if (maxLeft > threshold && maxLeft > val6) {     // Turn left
      ...
      if (distance < 12 && distance >= 8) {  // Throttle velocity based on ultrasonic distance reading (distances in cm)
        velocity = 0.5 * velocity;
      }
      else if (distance < 8 && distance > 1) {   //ADJUST VALUE OF DISTANCE BASED ON LEGNTH OF MAGNET ARM
        velocity = 0;
